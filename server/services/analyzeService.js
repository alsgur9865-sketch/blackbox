...
function avg(list) { return list.length ? list.reduce((sum, value) => sum + value, 0) / list.length : 0; }

export function analyzeInvestment(trades, answers) {
  const losses = trades.filter((trade) => trade.sellPrice < trade.buyPrice);
  const wins = trades.filter((trade) => trade.sellPrice >= trade.buyPrice);
  const chaseTrades = trades.filter((trade) => trade.preRisePct >= 10);
  const socialTrades = trades.filter((trade) => ['youtube', 'community', 'recommendation'].includes(trade.source));
  const lossHold = avg(losses.map((trade) => trade.holdingDays));
  const winHold = avg(wins.map((trade) => trade.holdingDays));
  const beginnerRisk = answers.experience === 'under1' ? 6 : answers.experience === '1to3' ? 2 : 0;

  let fomo = trades.length ? (chaseTrades.length / trades.length) * 78 : 0;
  if (answers.entryStyle === 'momentum') fomo += 14;
  if (answers.entryStyle === 'impulse') fomo += 22;
  ...
  if (answers.checkRoutine === 'sometimes') dependency += 4;
  dependency += beginnerRisk;
  ...
  if (answers.experience === 'under1' && answers.tradeFrequency === 'daily') revenge += 6;

  const patterns = [
    { key: 'fomo', name: 'FOMO 매수', score: clamp(fomo), desc: '단기 상승 이후 진입한 거래와 매수 성향, 투자 경험을 함께 분석했습니다.', action: '매수 전 24시간 대기 규칙과 진입 근거 2줄 기록을 적용해보세요.' },
  ...
  const riskAverage = avg(patterns.slice(0, 3).map((pattern) => pattern.score));
